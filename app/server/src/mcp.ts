 ...
 *
 *   { "command": "npm", "args": ["run", "mcp"], "cwd": "C:/Code/AiDailyTasks" }
 *
...
async function main(): Promise<void> {
  ...
  process.stderr.write(`AiDailyTasks MCP (stdio) ready — board: ${env.boardDir}\n`);
}

main().catch((err) => {
  process.stderr.write(`AiDailyTasks MCP failed to start: ${(err as Error).stack ?? err}\n`);
  process.exit(1);
});
